...
def conversion(y_train,dataset):
    import pandas as pd
    Actual_y_train=pd.DataFrame(index=range(len(y_train)),columns=dataset.columns)
...
def conversionSingle(y_train,dataset):
    import pandas as pd
    Actual_y_train=pd.DataFrame(index=range(len(y_train)),columns=dataset)
    for i in range(len(y_train)):
        Actual_y_train.iloc[i]=y_train[i]
    return Actual_y_train
